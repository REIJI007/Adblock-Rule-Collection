...
async def download_filters(urls):
    ...
    async with aiohttp.ClientSession() as session:
        tasks = [download_filter(url, session) for url in urls]
...
def validate_rules(rules):
    ...
    for rule in rules:
        if is_valid_rule(rule):
            ...
            logging.warning(f"Invalid rule detected: {rule}")
    return valid_rules
...
def main():
    ...
    logging.info("Filtering and validating rules...")
    ...
    write_rules_to_file(valid_rules, save_path)
